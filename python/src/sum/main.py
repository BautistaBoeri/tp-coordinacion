...
class SumFilter:
    ...
    def _process_data(self, client_id, fruit, amount, flush_publisher):
        with self._lock:
            ...
            by_fruit = self.amount_by_fruit_by_client[client_id]
    ...
    def start(self):
        flush_thread = threading.Thread(target=self._run_flush, name="sum-flush")
        flush_thread.start()
        try:
            self._run_data()
        finally:
